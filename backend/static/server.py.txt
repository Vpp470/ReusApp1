...
# Verificar si tenim un frontend vàlid (amb index.html)
has_valid_frontend = dist_path.exists() and index_path.exists()

if has_valid_frontend:
    logger.info(f"Mounting Expo web app from {dist_path}")
    
    # Muntar assets només si existeix
    assets_path = dist_path / "assets"
    if assets_path.exists():
        app.mount("/assets", StaticFiles(directory=str(assets_path)), name="app-assets")
    ...
    # Middleware per servir SPA - captura totes les rutes que no són API
    from starlette.middleware.base import BaseHTTPMiddleware
    from fastapi.responses import FileResponse
    
    class SPAMiddleware(BaseHTTPMiddleware):
        async def dispatch(self, request, call_next):
            # Si és una ruta API o landing, deixar passar
            if request.url.path.startswith("/api/") or request.url.path.startswith("/landing/"):
                return await call_next(request)
            
            # Si és un fitxer estàtic que existeix, servir-lo
            file_path = dist_path / request.url.path.lstrip("/")
            if file_path.is_file():
                response = FileResponse(file_path)
                # No cache per fitxers JS/HTML per evitar problemes de cache
            ...
            # Per totes les altres rutes, servir index.html (SPA routing)
            if index_path.exists():
                response = FileResponse(index_path)
                # No cache per index.html
                response.headers["Cache-Control"] = "no-cache, no-store, must-revalidate"
                response.headers["Pragma"] = "no-cache"
                response.headers["Expires"] = "0"
                return response
            
            # Si index.html no existeix, seguir amb el processament normal
            return await call_next(request)
    
    app.add_middleware(SPAMiddleware)
    logger.info("SPA middleware added")
else:
    logger.warning(f"dist directory or index.html not found at {dist_path}")

# SEMPRE definir una ruta de fallback per a / (per si el frontend no existeix o falla)
from fastapi.responses import HTMLResponse

@app.get("/", response_class=HTMLResponse)
async def root_page():
    # Si tenim frontend vàlid, redirigir a index.html
    if has_valid_frontend:
        return FileResponse(index_path)
    # Sinó, mostrar pàgina de manteniment
    ...
    <head>
        ...
        <style>
            body {{ font-family: Arial, sans-serif; text-align: center; padding: 50px; background: linear-gradient(135deg, #2d5016 0%, #4a7c23 100%); color: white; min-height: 100vh; margin: 0; }}
    ...
    <body>
        <div class="container">
            <div class="logo">🍷</div>
            <h1>El Tomb de Reus</h1>
            <p>L'aplicació està en mode API.</p>
            <p>Per accedir a l'app mòbil, descarrega-la des de les botigues d'aplicacions.</p>
            <p style="margin-top: 30px;"><a href="/api/health">✓ Verificar estat de l'API</a></p>
            <p><a href="/api/establishments">📍 Veure establiments</a></p>
            <p><a href="/api/events">📅 Veure esdeveniments</a></p>
        </div>
    </body>
    </html>
    """)
